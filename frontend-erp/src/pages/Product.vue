...
async function fetchProducts() {
  ...
  try {
    ...
    products.value = res.data.map((p) => ({
      ...p,
      cost: "Rp" + Number(p.cost).toLocaleString(),
      price: "Rp" + Number(p.price).toLocaleString(),
  ...
  >
    ...
    <template v-if="slideoutMode === 'add' || slideoutMode === 'edit'">
      <form class="flex flex-col gap-4">
        ...
        <MyInputField label="Price" v-model="form.price" placeholder="Price" />
      </form>
    </template>
